...
int point_sense(int i, npt N[], int mp[],int aux[], int h){
	int j;
	if(i==0){
		if(N[i+1]->val == '*' || N[i+h]->val == '*'){
			//mp[i] = 1;
			if(N[i+1]->val=='*' && aux[i+1]!=1) {mp[i+1] = mp[i] + mp[i+1]; aux[i+1] = 1;}
			else 
			{
				if(aux[i+h]!=1) {
				mp[i+h] = mp[i]+ mp[i+h];
		...
		if(N[i-1]->val == '*' || N[i+1]->val == '*' || N[i+h]->val == '*'){
			if(N[i-1]->val == '*'&&aux[i-1]!=1){ mp[i-1] = mp[i] + mp[i-1]; aux[i-1] = 1;}
			else if(N[i+1]->val == '*'&&aux[i+1]!=1){mp[i+1] = mp[i] + mp[i+1]; aux[i+1] = 1;}
			else{
				if(aux[i+h]!=1){
				mp[i+h] = mp[i] + mp[i+h];
				aux[i+h] = 1;
